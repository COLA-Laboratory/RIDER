...
class RNAGraphFeaturizer(object):
    ...
    def __call__(self, rna):
        with torch.no_grad():
            ...
            # )
            coords_list, mask_coords, mask_confs = get_one_fixed_entry_and_masks(rna['coords_list'], fixed_idx=0)
            ...
            # if self.split == 'test':
            #     coords_list += torch.randn_like(coords_list, device=self.device) * 0.1
...
def get_one_fixed_entry_and_masks(coords_list, fixed_idx=0):
    """
    只取 coords_list[fixed_idx] 这一条骨架，并返回它和对应的 mask。
    """
    coords_list = np.array(coords_list)
    k = 1

    confs_list =  coords_list[fixed_idx]
    confs_list = confs_list[None, ...]                       # (1, res, atoms, 3)
