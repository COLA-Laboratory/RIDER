...
def _resolve_x3dna_path(x3dna_path: Optional[str]) -> str:
    if x3dna_path:
        return x3dna_path
    if not X3DNA_PATH:
        raise EnvironmentError(
            "X3DNA path is not set. Export X3DNA to the X3DNA install root "
            "(e.g., export X3DNA=/path/to/x3dna) or pass x3dna_path explicitly."
        )
    return os.path.join(X3DNA_PATH, "bin/find_pair")


def _resolve_eternafold_path(eternafold_path: Optional[str]) -> str:
    if eternafold_path:
        return eternafold_path
    if not ETERNAFOLD_PATH:
        raise EnvironmentError(
            "ETERNAFOLD path is not set. Export ETERNAFOLD to the install root "
            "(e.g., export ETERNAFOLD=/path/to/EternaFold) or pass eternafold_path explicitly."
        )
    return os.path.join(ETERNAFOLD_PATH, "src/contrafold")
...
def pdb_to_sec_struct(
    ...
    """
    x3dna_path = _resolve_x3dna_path(x3dna_path)
...
def predict_sec_struct(
        ...
        eternafold_path: Optional[str] = None,
        n_samples: int = 1,
    ) -> str:
    """
    ...
    # Run EternaFold
    eternafold_path = _resolve_eternafold_path(eternafold_path)
    if n_samples > 1:
